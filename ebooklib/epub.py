...
class EpubHtml(EpubItem):
    ...
    def add_item(self, item: Literal[1, 2, 3, 4, 5, 6, 7, 8, 9]):
        ...
        if item.get_type() == ebooklib.ITEM_STYLE:
            self.add_link(
                href=item.get_name(), rel="stylesheet", type="text/css"
            )
...
class EpubBook(object):
    ...
    def set_unique_metadata(
        self, namespace: str, name: list, value: str, others: dict = None
    ) -> None:
        """Add metadata if metadata with this identifier does not already
        exist, otherwise update existing metadata."""
...
class EpubWriter(object):
    ...
    def _write_opf_metadata(self, root):
        ...
        #     if ns_name:
        #         for n_id, ns_url in iteritems(NAMESPACES):
        #             if ns_name == ns_url:
                ...
                if isinstance(item, tuple) or isinstance(item, list):
                    ...
                    if isinstance(item[0], EpubHtml):
                        a = etree.SubElement(
                            ...
                            "a",
                            {
                                "href": zip_path.relpath(
                                    item[0].file_name, nav_dir_name
                                )
                            },
                        )
                    ...
                    a = etree.SubElement(
                        li,
                        "a",
                        {"href": zip_path.relpath(item.href, nav_dir_name)},
                    )
                    a.text = item.title
...
class EpubReader(object):
    ...
    def _load_metadata(self):
        ...
        # get xml:lang
        # get metadata
        metadata = self.container.find(
            "{%s}%s" % (NAMESPACES["OPF"], "metadata")
        )

        nsmap = metadata.nsmap
        nstags = dict((k, "{%s}" % v) for k, v in iteritems(nsmap))
        ...
        for t in metadata:
            ...
            if t.tag == default_ns + "meta":
                ...
                if (
                    t.prefix and t.prefix.lower() == "dc"
                ) and tag == "identifier":
                    _id = t.get("id", None)
    ...
    def _load_manifest(self):
        for r in self.container.find(
            "{%s}%s" % (NAMESPACES["OPF"], "manifest")
        ):
            ...
            if media_type == "application/x-dtbncx+xml":
                ...
                ei.content = self.read_file(
                    zip_path.join(self.opf_dir, ei.file_name)
                )
    ...
    def _load_spine(self):
        ...
        nav_item = next(
            (item for item in self.book.items if isinstance(item, EpubNav)),
            None,
        )
        if toc:
            if not self.options.get("ignore_ncx") or not nav_item:
                try:
                    ncxFile = self.read_file(
                        zip_path.join(
                            self.opf_dir,
                            self.book.get_item_with_id(toc).get_name(),
                        )
                    )
                except KeyError:
